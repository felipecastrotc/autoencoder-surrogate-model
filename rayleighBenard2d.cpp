...
T Ra = 1e4;                                    // Rayleigh number
T Pr = 0.71;                                   // Prandtl number
T Thot = 274.15;                               // temperature of the lower wall in Kelvin
T Tcold = 273.15;                              // temperature of the fluid in Kelvin
T Tperturb = 1. / 5. * Tcold + 4. / 5. * Thot; // temperature of the perturbation
...
int main(int argc, char *argv[])
{
    ...
    if (argc > 1)
    {
        Ra = atof(argv[1]);    // Rayleigh number
        Pr = atof(argv[2]);    // Prandtl number
        Thot = atof(argv[3]);  // temperature of the lower wall in Kelvin
        Tcold = atof(argv[4]); // temperature of the fluid in Kelvin
        // Ra = atof(argv[0]);                          // Rayleigh number
        // Pr = atof(argv[1]);                          // Prandtl number
        // Thot = atof(argv[2]);                        // temperature of the lower wall in Kelvin
        // Tcold = atof(argv[3]);                       // temperature of the fluid in Kelvin
        Tperturb = 1. / 5. * Tcold + 4. / 5. * Thot; // temperature of the perturbation
        path = "./";
        path += argv[5];
        path += "/";
    }
    ...
    {
        argv[i] = argv[i + 6]; // copy next element left
    }
    /// === 1st Step: Initialization ===
    OstreamManager clout(std::cout, "main");
    olbInit(&argc, &argv);
    singleton::directories().setOutputDir(path);
